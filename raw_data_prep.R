...
# print the result
print(neg_sums)
### they are no number greater than or equal to 0. 


data_final <- data_final %>%
  select(-ICD10_cancer.0.0, -ICD9_cancer.0.0, -ICD10_main.0.0, -ICD9_main.0.0)
...
data_final <- data_final %>%
  select(-time_walking_min.0.0)

sum(data_final$diabetes..0.0 < 0, na.rm = T)

sum(data_final$sex.0.0 == 0 & data_final$diabetes.0.0 == 1, na.rm = TRUE)



# Remove rows where diabetes..0.0 is less than 0
data_final <- data_final[data_final$diabetes..0.0 >= 0 | is.na(data_final$diabetes..0.0),]

# only guys has diabetes here 
data_final_filtered <- data_final[data_final$sex.0.0 != 0, ]

sum(data_final_filtered$diabetes..0.0 == 1, na.rm = TRUE)

missing_count <-colSums(is.na(data_final_filtered))

# print the missing count for each column 
print(missing_count)

# remove more unneeded data 
data_final_filtered <- subset(data_final_filtered, select = -c(sex.0.0, rheumatoid_factor.0.0, oestradiol.0.0))
data_final_filtered <- subset(data_final_filtered, select = -c(age))
write.csv(data_final_filtered, "data_final.csv", row.names = TRUE)
